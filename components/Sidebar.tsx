...
export const Sidebar: React.FC<SidebarProps> = memo(({ activeFeature, setActiveFeature, onToggleDebug, chatLogic }) => {
  ...
  const { shouldShowNav, isForcedStealth } = useNavigationIntelligence();
  
  const isStealthMode = (!shouldShowNav && !isHovered) || isForcedStealth;
    ...
    <aside 
      ...
      className={`hidden md:flex flex-col h-full bg-white/40 dark:bg-[#080809]/95 backdrop-blur-xl border-r z-[500] transition-all duration-700 ease-[cubic-bezier(0.2,0,0,1)] relative group overflow-hidden border-black/5 dark:border-white/10 ${
        isForcedStealth 
          ...
          : (isStealthMode ? 'w-[8px] opacity-40' : (isHovered ? 'w-72 shadow-[20px_0_60px_rgba(0,0,0,0.4)]' : 'w-[80px]'))
      }`}
    >
      <div className="w-full flex flex-col h-full relative z-10">
        
        {/* Header Logo */}
        <div className={`h-20 flex items-center transition-all duration-500 ${isHovered ? 'px-6' : 'justify-center'} ${isStealthMode ? 'opacity-0' : 'opacity-100'}`}>
        ...
        {/* Navigation Links */}
        <div className={`flex-1 flex flex-col transition-all duration-500 overflow-hidden ${isStealthMode ? 'opacity-0' : 'opacity-100'} ${isHovered ? 'px-4' : 'px-3'}`}>
        ...
        {/* Footer Actions */}
        <div className={`mt-auto transition-all duration-500 ${isStealthMode ? 'opacity-0' : 'opacity-100'} ${isHovered ? 'p-5 space-y-1' : 'p-2 flex flex-col items-center gap-1 pb-8'}`}>

...
export const MobileNav: React.FC<MobileNavProps> = memo(({ activeFeature, setActiveFeature, onToggleDebug }) => {
  ...
  return (
    <div 
      className={`md:hidden fixed left-1/2 -translate-x-1/2 z-[100] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] will-change-transform ${
        isVisible 
    ...
    >
      <nav className={`
        ...
        rounded-[2.618rem] p-2 pl-5 pr-2 
      ...
      `}>
          ...
          {FEATURES.map((f) => {
            ...
            return (
              <button
                ...
                className={`relative w-[42px] h-[42px] rounded-full flex items-center justify-center transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                  isActive 
        ...
        {/* Divider */}
        <div className="w-[1px] h-[20px] bg-black/10 dark:bg-white/10 mx-2"></div>
        
        {/* Utilities */}
        <div className="flex items-center gap-1">
          <button 
            onClick={() => onToggleDebug?.()} 
            className="w-[42px] h-[42px] rounded-full flex items-center justify-center text-neutral-400 dark:text-neutral-500 hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 transition-all active:scale-90"
          >
            <Bug size={18} />
          </button>
          ...
          <button 
            ...
            className={`w-[42px] h-[42px] rounded-full flex items-center justify-center transition-all duration-500 ${
